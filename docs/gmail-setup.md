...
which ones got replies. To do that, the app needs two files at the project root,
both git-ignored:

| File | What it is | How you get it |
| --- | --- | --- |
| `credentials.json` | The OAuth *client* — identifies this app to Google. | Downloaded from Google Cloud (steps below). |
| `token.json` | Your *authorization* — lets the app act as you. | Minted by `scripts/gmail_auth.py`. |
...
3. In the dialog, **Download JSON**. Save it as `credentials.json` in the project
   root (next to `.env`).
...
past it.) On success the script writes `token.json` and prints the connected
account.
